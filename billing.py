...
class BillClass:
    def __init__(self, root):
       ...
       #=== search frame==
        ...
        lbl_search=Label(ProductFrame2,text="Product Name",font=("time new roman",15,"bold"),bg="white").place(x=2,y=45)
        txt_search=Entry(ProductFrame2,textvariable=self.var_search,font=("time new roman",15),bg="lightyellow").place(x=140,y=49,width=150,height=22)

        btn_search=Button(ProductFrame2,text="search",command=self.search,font=("goudy old style",15),bg="#2196f3",fg="white",cursor="hand2").place(x=290 , y=48,width=100,height=25)
        ...
        # self.ProductTable=ttk.Treeview(ProductFrame3,columns=("pid","price","name","qty","status"),yscrollcommand=scrolly.set,xscrollcommand=scrollx.set)
        ...
        # self.ProductTable.heading("contact",text="CONTACT")
        ...
        lbl_node=Label(ProductFrame1,text="Note : Enter 0  Quantity to remove from the Cart", font=("goudy old style",13),bg="white",fg="red").pack(side=BOTTOM,fill=X)
        ...
        coustomerFrame=Frame(self.root,bd=4,relief=RIDGE,bg="white")
        coustomerFrame.place(x=420,y=110,width=530,height=70)
        cTitle=Label(coustomerFrame,text="Customer Deltail",font=("goudy old style",15),bg="lightgray").pack(side=TOP,fill=X)
        lbl_name=Label(coustomerFrame,text="Name",font=("time new roman",15),bg="white").place(x=5,y=35)
        txt_name=Entry(coustomerFrame,textvariable=self.var_cname,font=("time new roman",13),bg="lightyellow").place(x=80,y=35,width=180)
      
        lbl_contect=Label(coustomerFrame,text="Contact no.",font=("time new roman",15),bg="white").place(x=270,y=35)
        txt_contect=Entry(coustomerFrame,textvariable=self.var_contact,font=("time new roman",13),bg="lightyellow").place(x=380,y=35,width=140)
        ...
        txt_cal_imput=Entry(Cal_Frame,textvariable=self.var_cal_input,font=('arial',15,'bold'),width=21,bd=10,relief=GROOVE,state='readonly',justify=RIGHT)
        txt_cal_imput.grid(row=0,columnspan=4)
        ...
        #self.CartTable.heading("contact",text="CONTACT")
        ...
        # btn_add_cart=Button(Add_cartwidgetsFrame,text="Add|Update",font=("times new roman",15,"bold"),bg="lightgray",cursor="hand2").place(x=180 , y=70,width=150,height=30)
        ...
        self.lbl_amnt=Label(billMenuFrame,text='Bill Amount\n[0]',cursor="hand2",font=("goudy old style",15,"bold"),bg="#3f51b5",fg="white")
        self.lbl_amnt.place(x=2,y=5,width=120,height=70)
       
        self.lbl_discount=Label(billMenuFrame,text='Discount\n[5%]',cursor="hand2",font=("goudy old style",15,"bold"),bg="#8bc34a",fg="white")
        self.lbl_discount.place(x=124,y=5,width=120,height=70)
       
        self.lbl_net_pay=Label(billMenuFrame,text='Net Pay\n[0]',cursor="hand2",font=("goudy old style",15,"bold"),bg="#607d8b",fg="white")
        ...
        # ========= footer ===
        footer=Label(self.root , text="IMS - Inventory Management  ",font=("times new roman",11),bg="#4d636d",fg="white").pack(side=BOTTOM,fill=X)
...
#====   all function of calculator ==================
    ...
    def search(self):
        ...
        try:
            if self.var_search.get()=="":
                messagebox.showerror("Error","Search input should be requried",parent=self.root)
            else:
    ...
    def add_update_cart(self):
        if  self.var_pid.get()=='':
            ...
            for row in self.cart_list:
                ...
                index_+=1
            if present=='yes':
                op=messagebox.askyesno('Confirm',"product already present \n Do yoy want to update | Remove from the cart",parent=self.root)
                if op==True:
    ...
    def generate_bill(self):
        if self.var_cname.get()=='' or self.var_contact.get()=='':
            messagebox.showerror("Error",f"Customer Details are requid",parent=self.root)
            ...
            fp.close()
            messagebox.showerror('Save',"Bill has been generated/Save in backed",parent=self.root)
            self.chk_print=1
    ...
    def bill_top(self):
        ...
        bill_top_temp=f'''
\t\tXYZ-Inventory
\t Phone No. 620565295 , Delhi-125001
{str("="*47)}
    ...
    def print_bill(self):
        if self.chk_print == 1:
            try:
                ...
                new_file = tempfile.NamedTemporaryFile(suffix=".txt", delete=False).name
                with open(new_file, 'w') as file:
                    file.write(self.txt_bill_area.get('1.0', END))
                os.startfile(new_file, 'print')
            except Exception as e:
                messagebox.showerror('Error', f"An error occurred while printing: {str(e)}", parent=self.root)
        else:
